...
### 文章阅读
- 📋 三栏布局：侧边栏 → 文章列表 → 阅读视图
- 📱 **响应式设计** — 移动端侧边栏浮层滑入，文章列表与阅读视图全屏切换
- 👁️ 未读 / 已读追踪，一键标记
- ⭐ 文章收藏，独立收藏筛选
...
### 移动端响应式
- 📱 **响应式布局** — 自动适配手机、平板、桌面
- 🍔 **可隐藏侧边栏** — 订阅源列表在移动端变为浮层叠加，通过汉堡按钮开关
- 📖 **全屏阅读** — 文章列表与阅读视图在移动端独占全屏，返回按钮回到列表
- 🧹 **响应式工具栏** — 小屏自动隐藏不常用操作，保留核心按钮
...
Kingbird 是**纯客户端工具**，不托管、存储、分发或推荐任何内容。所有数据仅保存在用户本地设备，不上传至任何服务器。用户自行对其订阅的 RSS 源及访问的内容负责。本项目与任何内容源无关，不提供代理、绕过审查或翻墙功能。
